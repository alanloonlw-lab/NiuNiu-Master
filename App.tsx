...
const App: React.FC = () => {
  ...
  return (
    <ErrorBoundary>
      ...
      <div className={`h-screen w-full overflow-hidden flex flex-col items-center bg-casino-felt text-white font-sans transition-all duration-700 ${isCriticalTimer ? 'animate-shake vignette-critical' : ''}`}>
        <header className="w-full bg-black/60 backdrop-blur-2xl border-b border-white/5 shrink-0 z-50 relative px-2 sm:px-4 py-1.5 sm:py-3">
        ...
        <main className="flex-1 w-full max-w-7xl mx-auto flex flex-col justify-center gap-2 sm:gap-6 items-center px-2 sm:px-8 relative py-2 overflow-hidden">
          {/* Information Toggle Button - Table Top Left */}
          <button 
            onClick={() => { sounds.playSelect(); setIsRulesOpen(true); }}
            className="absolute left-2 top-2 sm:left-4 sm:top-4 z-[60] flex items-center gap-2 bg-black/40 border border-yellow-500/50 text-yellow-500 px-2 py-1.5 sm:px-3 sm:py-2 rounded-full backdrop-blur-md shadow-lg hover:bg-yellow-500/20 hover:scale-105 transition-all animate-pulse duration-[3s]"
          >
          ...
          {/* Top Section: Dealer Cards */}
          <div className="w-full flex flex-col items-center relative shrink-0">
            <div className="text-[8px] sm:text-[12px] font-black text-stone-500 uppercase tracking-[0.4em] mb-2 sm:mb-4 flex items-center gap-3 sm:gap-6">
              <span className="w-6 sm:w-16 h-[1px] bg-stone-800"></span>
              BANKER
              <span className="w-6 sm:w-16 h-[1px] bg-stone-800"></span>
            </div>
            {phase === GamePhase.RESULT && dealerResult && (
               <div className="absolute -top-4 sm:-top-8 bg-red-600 text-white px-4 py-1 sm:px-6 sm:py-2 rounded-full text-[10px] sm:text-[14px] font-black cinzel z-20 animate-in zoom-in shadow-2xl border border-white/20 ring-4 ring-black/50">{dealerResult.type}</div>
            )}
          ...
          {/* Middle Section: Game Status (Timer/Result Message) */}
          <div className="h-24 sm:h-40 flex flex-col items-center justify-center text-center z-20 shrink-0 w-full">
            {phase === GamePhase.SELECTING ? (
              ...
              <div className="flex flex-col items-center gap-2 sm:gap-6">
                <Logo className="opacity-10 scale-50 sm:scale-125 grayscale" />
          ...
          <div className="w-full flex flex-col items-center relative shrink-0">
            {phase === GamePhase.RESULT && playerResult && (
               <div className="absolute -top-4 sm:-top-8 bg-yellow-500 text-black px-4 py-1 sm:px-6 sm:py-2 rounded-full text-[10px] sm:text-[14px] font-black cinzel z-20 animate-in zoom-in shadow-2xl border border-black/10 ring-4 ring-black/50">{playerResult.type}</div>
            )}
            <div className="flex gap-1.5 sm:gap-4 justify-center items-center">
            ...
            <div className="text-[8px] sm:text-[12px] font-black text-stone-500 uppercase tracking-[0.4em] mt-2 sm:mb-2 flex items-center gap-3 sm:gap-6">
               <span className="w-6 sm:w-16 h-[1px] bg-stone-800"></span>
        ...
        <footer className="w-full max-w-5xl mx-auto bg-black/80 px-4 py-4 sm:px-10 sm:py-6 rounded-t-[2rem] sm:rounded-t-[3rem] border-t border-white/10 shrink-0 shadow-[0_-20px_50px_rgba(0,0,0,0.8)] backdrop-blur-3xl z-40">
          {phase === GamePhase.BETTING ? (
            <div className="flex flex-col gap-3 sm:gap-4">
              <div className="flex overflow-x-auto sm:flex-wrap justify-start sm:justify-center gap-2 sm:gap-3 pb-2 sm:pb-0 no-scrollbar">
                {BET_INCREMENTS.map(val => (
                  <button 
                    key={val} 
                    disabled={balance < (bet + val)} 
                    className="shrink-0 px-3 py-2 sm:px-6 sm:py-3 bg-white/5 border border-white/10 rounded-xl sm:rounded-2xl font-black transition-all hover:bg-white/10 hover:border-white/20 disabled:opacity-10 text-[10px] sm:text-sm active:scale-90 shadow-2xl" 
                ...
                <button 
                  className="shrink-0 px-3 py-2 sm:px-6 sm:py-3 bg-red-900/20 text-red-500 border border-red-500/20 rounded-xl sm:rounded-2xl font-black text-[10px] sm:text-sm active:scale-90 shadow-2xl hover:bg-red-900/30" 
                  onClick={() => setBet(0)}
                >
                  RESET
                </button>
              ...
              <button 
                className={`w-full py-3 sm:py-5 text-lg sm:text-2xl font-black rounded-xl sm:rounded-[1.5rem] hover:brightness-110 active:scale-[0.97] cinzel disabled:grayscale disabled:opacity-30 transition-all red-gradient shadow-[0_10px_30px_rgba(185,28,28,0.4)] tracking-tighter uppercase`} 
                onClick={handleDeal} 
            ...
            <button 
              className={`w-full py-4 sm:py-6 text-lg sm:text-2xl font-black rounded-xl sm:rounded-[1.5rem] cinzel transition-all shadow-[0_10px_30px_rgba(234,179,8,0.3)] tracking-tighter active:scale-[0.97] uppercase ${selectedIndices.length === 3 ? 'gold-gradient text-stone-900' : 'bg-stone-800/50 text-stone-600 cursor-not-allowed'}`} 
            ...
            <button 
              className={`w-full py-4 sm:py-6 text-lg sm:text-2xl font-black rounded-xl sm:rounded-[1.5rem] cinzel transition-all shadow-2xl tracking-tighter active:scale-[0.97] uppercase ${isGameOver ? 'gold-gradient text-stone-900' : 'bg-white/5 text-white hover:bg-white/10 border border-white/10'}`} 
              onClick={() => { 
                if (isGameOver) {
                  setShowNameModal(true);
                } else {
